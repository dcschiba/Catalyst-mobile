...
WRAP.DH = {
    ...
    Reference: function(key, root) {
        ...
        this.getData = function(content, cb) {
            if ( !cb )
                return;
            if ( this._data && this._data.handler ) {
                var dh = this._data.handler;
                if ( dh.getData && dh.loadData) {
                    var gd = dh.getData(content);
                    if ( gd ) {
                        cb(gd);
                        return;
                    }
                    dh.loadData(content, null, function() {
                        var gd = dh.getData(content);
                        if ( gd ) {
                            cb(gd);
                        }
...
WRAP.DH.DataHandler.GeoTiff = function(reference, conf, base) {
    ...
    self.wrap_round = (self.grid.LonInterval*self.grid.Width >= 360.0);
...
//console.log("loadTile key="+key);
            ...
            var sep = Math.pow(2,tz);
            var ts = self.grid.TileSize*sep;
            var tileHeight = self.grid.LatInterval*ts;
            var tileWidth = self.grid.LonInterval*ts;
            var h = Math.ceil(self.grid.Height/ts);
            var w = Math.ceil(self.grid.Width/ts);
            for ( var y = 0 ; y < h ; y ++ ) {
                ...
                for ( var x = 0 ; x < w ; x ++ ) {
                    ...
                    if ( ex < sx )
                        ex += 360;
                    if ( (s_lon <= ex && sx < e_lon) || (s_lon+360 <= ex && sx < e_lon+360) ) {
                        var data_file = filePath(content, {z:tz, y:y, x:x});
                        for ( var i = 0 ; i < data_file.length ; i++ ) {
                            //console.log("tile="+data_file[i]);
                            var data = self.data_cache.get(data_file[i].file);
                            var active_width = self.grid.TileSize;
                            var active_height = self.grid.TileSize;
                            if ( (x+1)*ts > self.grid.Width )
                                active_width = Math.floor((self.grid.Width - x*ts)/sep);
                            if ( (y+1)*ts > self.grid.Height )
                                active_height = Math.floor((self.grid.Height - y*ts)/sep);
                            data_list.push({
...
//console.log("elemen="+eee+" index="+data_file[i].index+" Max="+maxValue+" Min="+minValue);
                data_list.push({
                    ...
                    active_width:self.grid.Width,
                    active_height:self.grid.Height,
                    n:self.grid.LatBase,
                    w:self.grid.LonBase,
                    s:self.grid.LatBase-self.grid.LatInterval*self.grid.Height,
                    e:self.grid.LonBase+self.grid.LonInterval*self.grid.Width,
...
//console.log("loadCompelte key="+key);
                if ( option.indexOf("value") >= 0 ) {
                    ...
                    for ( var i = 0 ; i < data_list.length ; i++ ) {
                        ...
                        var sy;
                        if ( d.latInterval > 0 )
                            sy = Math.round((d.n-90.0)/d.latInterval);
                        else
                            sy = Math.round((d.n+90.0)/d.latInterval);
                        // adjust to global offset
                        while ( sy > 0 )
                            sy -= step;
                        while ( sy < 0 )
                            sy += step;
                        var sx = Math.round((-d.w)/d.lonInterval);
                        while ( sx > 0 )
                            sx -= step;
                        while ( sx < 0 )
                            sx += step;
                        
                        for ( yi = sy ; yi < d.active_height ; yi+=step ) {
                            var lat = d.n - d.latInterval*yi;
                            if ( lat > n || s >= lat )
                                continue;
                            for ( xi = sx ; xi < d.active_width ; xi+=step ) {
                ...
                if ( option == "interpolate") {
                        ...
                        for ( var i = 0 ; i < data_list.length ; i++ ) {
                            ...
                            if ( 0 <= yi && yi < d.height ) {
                                ...
                                if ( 0 <= xi && xi < d.width ) {
                                    ...
                                    if ( yi+1 < d.height ) {
                                        var lt = vv;
                                        var rt = v[d_index+offset];
                                        var lb = v[d_index+d.width];
                                        var rb = v[d_index+d.width+offset];
                                        var yr = yp-yi;
                                        var xr = xp-xi;
                                        vv = lt*(1.0-yr)*(1.0-xr)
                                           + rt*(1.0-yr)*xr
                    ...
                    for ( var p = 0 ; p < num ; p++ ) {
                        ...
                        for ( var i = 0 ; i < data_list.length ; i++ ) {
                            ...
                            var out = (d.element <= 1)?tile.data:tile.data[d.index];
                            var v = d.data.data;
                            var yi = Math.round((d.n-lat)/d.latInterval);
                            if ( 0 <= yi && yi < d.height ) {
                                var xp = ((lon >= d.w)?(lon-d.w):(lon-(d.w-360)))/d.lonInterval;
                                var xi = Math.round(xp);
                                if ( self.wrap_round)
                                    xi %= d.width;
                                if ( 0 <= xi && xi < d.width ) {
                                    var d_index = yi*d.width+xi;
    ...
    this.getTileList = function(content, context) {
        ...
        if ( self.size > 0 ) {
            ...
            for ( var i = 0 ; i < context.tiles.length ; i++ ) {
                ...
                var e_lat = cood[256*256*2-2];
                var e_lon = cood[256*256*2-1];
                //console.log("tile id="+tile.id+" slat="+s_lat+" s_lon="+s_lon+" e_lat="+e_lat+" e_lon="+e_lon);
...
WRAP.DH.DataHandler.IndexedGeoJSON = function(reference, conf, base) {
    
    var self = this;
    
    self.name = conf.Name;
    self.indexfile = conf.Attributes.Index;
    self.datafile = conf.Attributes.File;
    ...
    this.load = function(ref, cb) {
        if ( self.indexfile ) {
            WRAP.DH._getJSON(self.indexfile, "Index", function(data) {
                if ( data.index ) {
                    base.index = data;
                    base.data = {};
console.log("loaded index:"+self.indexfile);
                    var index = data.index;
                    if ( index ) {
                        for ( var id in index ) {
                            var url = self.datafile.replace("%id%", id);
                            WRAP.DH._getJSON(url, "Indexed GeoJSON", function(data) {
                                var data_id = data.id;
                                if ( data_id ) {
                                    base.data[data_id] = data;
                                    index[data_id].loaded = true;
console.log("loaded data:"+data_id);
...
console.log("load completed");
                    ...
                    WRAP.DH._error(self.name + " Index format.");
                    cb("error", null);
                }
