...
WRAP.Geo = {
    ...
    /**
     * 任意地点から指定距離、指定方向の地点座標を返す
     * @method getGCPoint
     * @param {WRAP.Geo.Point} point
     * @param {Number} 距離（m）
     * @param {Number} 方位（degree）
     * @return {WRAP.Geo.Point} target_point
     **/
    getGCPoint : function(point, distance, direction) {
        if ( distance <= 0 )
            return point;
        distance /= 1851.8518518518517;
        var pos = WRAP.Geo.gcPos(point.latDegree(), point.lonDegree(), direction, distance);
        return new WRAP.Geo.Point(pos.lat*60, pos.lon*60);
    },
    ...
    Layer: function(name) {
        ...
        this.setVisible = function(flag) {
            ...
            if ( this.blended ) {
                for ( var i = 0 ; i < this.blended.length ; i++ ) {
                    this.blended[i].invalidate();
                }
            }
            var b = WRAP.Geo.isBlended(this);
            if ( b )
                b.invalidate();
         ...
         **/
        this.clear = function() {
            if ( WRAP.Geo.map.clearDisplayCache )
                WRAP.Geo.map.clearDisplayCache(this._dc);
            this._dc = {};
            this._dl = [];
            WRAP.Geo.invalidate();
        }
        ...
        /**
         * レイヤーのデータをブレンドするレイヤーを登録する。
         * @method blend
         * @param {Array}　layers　データをブレンドするレイヤーの配列（nullまたは空配列の場合登録が解除される）
         * @return なし
         **/
        this.blend = function(layers) {
            if ( this.blended == layers )
                return;
            if ( this.blended && layers && this.blended.toString() == layers.toString() )
                return;
            this.blended = layers;
            this._dl = [];
            this.invalidate();
        ...
        this.render = function() {
            ...
            if ( this._visible && this._renderer && this._renderer.draw ) {
                var data = this._data;
                if ( data && !data._handler() )
                    return;
                if ( WRAP.Geo.isBlended(this) ) {
                    this._revision.data++;
                    this._revision.conf++;
                    this._dl = [];
                    return;
...
//console.log("renderMerge "+layer.name());
    ...
    Context: function(map) {
        ...
        this.setTileBand = function(zoom, min_tile_x, max_tile_x, min_tile_y, max_tile_y) {
            ...
            var tb = Math.pow(2,Math.floor(zoom));
            var racio = this.tile_band/tb;

            this.offset_x = Math.floor((this.min_tile_x-tb/2)*256.0*racio);
            this.offset_y = Math.floor((tb/2-1-this.max_tile_y)*256.0*racio);
            ...
            if ( this.max_tile_x < this.min_tile_x )
                tile_x = (tb-this.min_tile_x+this.max_tile_x+1);
            this.canvas.width = Math.floor(tile_x*256*racio);
            this.canvas.height = Math.floor(tile_y*256*racio);
            var z = Math.floor(zoom);
            for ( var yc = 0 ; yc < tile_y ; yc++ ) {
                var y = this.max_tile_y-yc;
                for ( var xc = 0 ; xc < tile_x ; xc++ ) {
                    var x = (this.min_tile_x+xc)%tb;
                    ...
                    tile.ctx = this.ctx;
                    tile.offset_x = Math.floor(xc*256*racio);
                    tile.offset_y = Math.floor(yc*256*racio);
                    tile.width = Math.floor((xc+1)*256*racio)-tile.offset_x;
                    tile.height = Math.floor((yc+1)*256*racio)-tile.offset_y;
                    tile.racio = racio;
                }
            }
        }
        ...
        this.hit = function(x, y, id) {
            ...
            while ( offset.x >= this.canvas_band )
                offset.x -= this.canvas_band;
            while ( offset.x < 0 )
                offset.x += this.canvas_band;
            x -= offset.x;
            y -= offset.y;
...
//console.log("r="+pd.data[0]+" g="+pd.data[1]+" b="+pd.data[2]+" id="+draw_id);
        ...
        this.drawArc = function(point, radius, start, end, lineWidth, strokeStyle, fillStyle, fillImage, id) {
            ...
            if ( !fillImage || c.loaded ) {
                ...
                for ( var r = start ; r < end ; r+=step ) {
                    var pos = WRAP.Geo.gcPos(point[1], point[0], r, radius);
                    path.push([pos.lon, pos.lat]);
                }
                var pos = WRAP.Geo.gcPos(point[1], point[0], end, radius);
                path.push([pos.lon, pos.lat]);
                ...
                if ( circle ) {
                    ...
                    if ( fillStyle || c.image ) {
                        var polygons = [];
                        path.push(point);
                        path.unshift(point);
                        var fragments = this._fragmentPath(path);
                        this._addFragments(polygons, fragments);
                        this._drawLines(polygons, lineWidth, null, null, fillStyle, c.image, null, id);
                    }
                    this._drawLines(lines, lineWidth, null, strokeStyle, null, null, null, id);
                }
            }
        };
        ...
        this.drawText = function(point, text, fontSize, fillStyle, strokeStyle, offset_x, offset_y, align, rotation) {
            ...
            var op = map.getScreenPoint(new WRAP.Geo.Point(0,0));
//console.log("SCREEN 0,0 = "+op.x+","+op.y);
            var tpx = this.offset_x+op.x;
            var tpy = this.offset_y+op.y;
            
            var p = map.getScreenPoint(new WRAP.Geo.Point(point[1]*60.0, point[0]*60.0));
//console.log("SCREEN "+point[0]+","+point[1]+"= "+op.x+","+op.y);
            var x = p.x-tpx;
            var y = p.y-tpy;
        ...
        this.drawTile = function(tile, imageData) {
            this.tile(function(draw_tile) {
                if ( draw_tile.id == tile.id ) {
                    ...
                    draw_tile.ctx.drawImage(canvas, draw_tile.offset_x, draw_tile.offset_y, tile.width, tile.height);
//console.log("drawTile "+tile.id+" x="+draw_tile.offset_x+" y="+draw_tile.offset_y+" w="+tile.width+" h="+tile.height);
                }
            });
        };
...
//        }
    ...
    renderContext: function(force_update) {
        var self = this;
...
//console.log("bounds changed n="+(bounds.north/60)+" s="+(bounds.south/60)+" e="+(bounds.east/60)+" w="+(bounds.west/60));
        ...
        var context_updated = (this.lastContext != this.map.context.revision);
    ...
    isBlended: function(layer) {
        var u = WRAP.Geo.container.upper_layers;
        if ( !u )
            return false;
        for ( var j = 0 ; j < u.length ; j++ ) {
            if ( u[j].visible() && u[j].blended && u[j].blended.indexOf(layer) >=0 )
                return u[j];
        }
        return false;
    },
    ...
    hit: function(point) {
        ...
        var band = context.canvas_band;
        var x = point.x;
        var y = point.y;
        var xx = [0];
        for ( var s = 1 ; x-band*s > -band ; s++ )
            xx.push(-band*s);
        for ( var s = 1 ; x+band*s < size.width+band ; s++ )
            xx.push(band*s);
        if ( this.container && this.container.upper_layers ) {
            for ( var i = 0 ; i < this.container.upper_layers.length ; i++ ) {
                ...
                if ( l.visible() ) {
                    for ( var b = 0 ; b < xx.length ; b++ ) {
                        var dx = xx[b];
                        x = point.x + dx;
                        for ( var j = l._dl.length-1 ; j >= 0  ; j-- ) {
                            var f = l._dl[j];
                            var h = f.hit(x, y, context);
                            if ( h ) {
                                return { layer:l, feature:f, x:h.x-dx, y:h.y, pixel:h.pixel };
                            }
                        }
                    }
                }
    ...
    gcPos: function(lat, lon, angle, distance) {
        var dist = distance;
        var PI_MIN = 10800;
    ...
    _tileCheck: function(context, layer) {
        var dl = layer._dl;
...
WRAP.Geo.Bridge.OpenLayers = function(map) {
    ...
    this.updateBounds = function(changing) {
        ...
        if ( ll[0] < -180 ) {
            while ( ll[0] < -180 ) {
                ll[0] += 360.0;
                this.view.setCenter(ol.proj.transform(ll, 'EPSG:4326', 'EPSG:3857'));
            }
        }
        if ( ll[0] >= 180 ) {
            while ( ll[0] >= 180 ) {
        ...
        var band = Math.pow(2,self.zoom)*256;
        if ( size[0] >= band )
            self.bounds = new WRAP.Geo.Bounds(nw[1]*60, se[1]*60, 180*60, -180*60);
        else
            self.bounds = new WRAP.Geo.Bounds(nw[1]*60, se[1]*60, se[0]*60, nw[0]*60);
        var zoom = Math.floor(self.zoom);
        ...
        var min_x = Math.floor(nwp[0]*s);
        var max_x = Math.floor((sep[0]==256?255:sep[0])*s);
        var min_y = Math.floor(sep[1]*s);
        var max_y = Math.floor(nwp[1]*s);
...
//            return;
        ...
        var extent = ol.proj.get("EPSG:3857").getExtent();
        var tile_band = Math.pow(2,parseInt(tile.z));
        var ff = Math.floor(this.context.canvas_band);
        
        function canvasPos(tile_x, tile_y, tile_band, extent) {
            var xx = (parseFloat(tile_x))/tile_band;
            var yy = (parseFloat(tile_y+1))/tile_band;
            xx = xx*(extent[2]-extent[0])+extent[0];
            yy = yy*(extent[3]-extent[1])+extent[1];
            ...
            //console.log("tile="+id+" pos="+(src[0]-dst[0])+","+(src[1]-dst[1]));
            
            var x = Math.round(src[0])-Math.floor(dst[0]);
            var y = Math.round(src[1])-Math.floor(dst[1]);
            return {x:x, y:y};
        }
        
        var c_pos = canvasPos(tile.x, tile.y, tile_band, extent);
        var n_pos = canvasPos(tile.x+1, tile.y-1, tile_band, extent);
        var x = c_pos.x;
        var y = c_pos.y;
...
//        if ( tile.x == tile_band-1 )
//            tw = this.context.canvas.width - x + 1;

        var sx = Math.floor(tile.offset_x);
        var sy = Math.floor(tile.offset_y);
        var sw = Math.floor(256*tile.racio);
        var sh = Math.floor(256*tile.racio);
        
//console.log("tile="+tile.id+" sx="+sx+" sy="+sy+" sw="+sw+" sh="+sh+" x="+x+" y="+y+" tw="+tw+" th="+th);
        
        self.canvas.ctx.clearRect(x*pr, y*pr, tw*pr, th*pr);
        self.canvas.ctx.drawImage(tile.canvas, sx, sy, sw, sh, x*pr, y*pr, tw*pr, th*pr);

        var s = 1;
        var tx;
        while ( (tx = x - ff*s) > -256 ) {
            self.canvas.ctx.clearRect(tx*pr, y*pr, tw*pr, th*pr);
            self.canvas.ctx.drawImage(tile.canvas, sx, sy, sw, sh, tx*pr, y*pr, tw*pr, th*pr);
            //console.log(" draw< "+tile.offset_x+","+tile.offset_y+" <= "+(x-ff*s)+","+y);
            s++;
        }
        s = 1;
        while ( (tx = x + ff*s) < self.canvas.width ) {
            self.canvas.ctx.clearRect(tx*pr, y*pr, tw*pr, th*pr);
            self.canvas.ctx.drawImage(tile.canvas, sx, sy, sw, sh, tx*pr, y*pr, tw*pr, th*pr);
            //console.log(" draw> "+tile.offset_x+","+tile.offset_y+" => "+sx+","+y);
            s++;
        }
/*
...
//        console.log("mouse="+point[0]+","+point[1]+" pos="+ll[1]+","+ll[0]);
    ...
    var canvasFunction = function(extent, resolution, pixelRatio, size, projection) {
        ...
        if ( last_canvas ) {
            var src = self.map.getPixelFromCoordinate([last_canvas.extent[0], last_canvas.extent[3]]);
            var dst = self.map.getPixelFromCoordinate([extent[0], extent[3]]);
            
            var src_w = last_canvas.extent[2]-last_canvas.extent[0];
            
            var dst_w = extent[2]-extent[0];
...
//            console.log("src="+src[0]+","+src[1]);
//            console.log("dst="+dst[0]+","+dst[1]);
//            console.log("dx="+dx+" dy="+dy);
            
            var sw = last_canvas.width;
            var sh = last_canvas.height;
            var dw = self.canvas.width*dps;
            var dh = self.canvas.height*dps;
  
            self.zoom = self.view.getZoom();
            var band = Math.pow(2,self.zoom)*256;
            var ox = [dx];
            var shift = band;
            while ( dx+shift < self.canvas.width ) {
                ox.push(dx+shift);
                shift += band;
            }
            shift = -band;
            ...
            for ( var i = 0 ; i < ox.length ; i++ ) {
                self.canvas.ctx.clearRect(ox[i], dy, dw, dh);
                self.canvas.ctx.drawImage(last_canvas, 0, 0, sw, sh, ox[i], dy, dw, dh);
            }
        }
...
/*
        ...
        console.log("canvasFunc cvs extent "+extent[0]+","+extent[1]+"->"+extent[2]+","
                    +extent[3]+" 0-2="+(extent[2]-extent[0])+" 1-3"+(extent[1]-extent[3]));
        console.log("canvasFunc map extent "+mapExtent[0]+","+mapExtent[1]+"->"+mapExtent[2]+","
                    +mapExtent[3]+" 0-2="+(mapExtent[2]-mapExtent[0])+" 1-3"+(mapExtent[1]-mapExtent[3]));
//console.log("canvasFunc mapExtent "+mapExtent[0]+","+mapExtent[3]);
        console.log("canvasFunc canvasOrigin "+canvasOrigin[0]+","+canvasOrigin[1]);
        console.log("canvasFunc mapOrigin "+mapOrigin[0]+","+mapOrigin[1]);
        console.log("canvasFunc delta "+delta[0]+","+delta[1]);
...
*/
    ...
    this.getScreenLine = function(path, n, s) {
        var lines = [];
        ...
        var line = [];
        for ( var i = 0 ; i < path.length ; i++ ) {
            var cood = ol.proj.transform([path[i][1], path[i][0]], 'EPSG:4326', 'EPSG:3857');
            var point = self.map.getPixelFromCoordinate(cood);
            var x = point[0];
            if ( min_x > x )
                min_x = x;
            if ( max_x < x )
                max_x = x;
            line.push(new WRAP.Geo.ScreenPoint(point[0], point[1]));
        }
        
        var band = self.context.canvas_band;
        var op = self.getScreenPoint(new WRAP.Geo.Point(0,0));
        var cx = self.map.getSize()[0]/2;
        while ( op.x-cx >= band/2 )
            op.x -= band;
        while ( op.x-cx < -band/2 )
            op.x += band;

        var sx = op.x+self.context.offset_x;
        var ex = sx+band;
        if ( sx < max_x && min_x < ex ) {
            lines.push(line);
        }
        var offset = -band;
        while ( max_x+offset > sx ) {
            if ( sx < max_x+offset && min_x+offset < ex ) {
                var shift = [];
                for ( var i = 0 ; i < line.length ; i++ )
                    shift.push(new WRAP.Geo.ScreenPoint(line[i].x+offset, line[i].y));
                lines.push(shift);
            }
            offset -= band;
        }
        offset = band;
        while ( min_x+offset < ex ) {
            if ( sx < max_x+offset && min_x+offset < ex ) {
                var shift = [];
                for ( var i = 0 ; i < line.length ; i++ )
                    shift.push(new WRAP.Geo.ScreenPoint(line[i].x+offset, line[i].y));
    ...
    this.getPerspective = function(pts, margin) {
        ...
        lat = center[1];
        lon = center[0];
        var factor = 128.0/20037508.342789244;    // normalize factor -128 -> +128
        lt = [lt[0]*factor,lt[1]*factor];
        rb = [rb[0]*factor,rb[1]*factor];
...
WRAP.Geo.setWebGLMap = function(map) {
    
    WRAP.Geo.map = new WRAP.Geo.Bridge.WebGLMap(map);
    
    return WRAP.Geo.map;
}

WRAP.Geo.Bridge.WebGLMap = function(map) {
    var self = this;
    var gl = null;
...
//    var gp = WRAP.Geo.WebGL;
    
    this.setProjection = function(name) {
        this.projection = new WRAP.Geo.WebGL.Projection[name](gl);
        this.projection.setZoom(this.zoom);
        this.projection.setCenter(this.center);
        this.context = new this.projection.Context(this.projection, gl);
    }
    
    this.clearDisplayCache = function(dc) {
        if ( dc ) {
            if ( dc.list ) {
                for ( var i = 0 ; i < dc.list.length ; i++ ) {
                    var e = dc.list[i];
                    if ( e.shader && e.vbo )
                        e.shader.deleteBuffer(e.vbo);
                }
                dc.list = [];
            }
            dc.cached = false;
...
/*
        ...
        gp.clear();
        gp.setOrthogonalMatrix(-w*0.5,w*0.5,-h*0.5,h*0.5,-100.0,100.00);
        gl.viewport(0, 0, w, h);
        ...
        var vbo = this.projection.colorShader.createBuffer();
        var vbo2 = this.projection.colorShader2.createBuffer();
        var vts = [
            140,30, 1.0,0.0,0.0,1.0, 5.0,
            160,30, 0.0,1.0,0.0,1.0, 10.0,
            140,40, 0.0,0.0,1.0,1.0, 2.0,
            160,40, 0.0,1.0,1.0,1.0, 5.0,
        ...
        this.projection.colorShader.set(vbo, vts);
//        this.projection.colorShader2.set(vbo2, vts2);
        this.projection.colorShader.drawTriangleStrip(gp.matrix, vbo, [{index:0, num:4}]);
//        this.projection.colorShader2.drawTriangleStrip(gp.matrix, vbo2, [{index:0, num:4}]);
        
        gl.flush();
        
        
        this.projection.colorShader.deleteBuffer(vbo);
        this.projection.colorShader2.deleteBuffer(vbo2);
*/
    }
    ...
    this.updateBounds = function() {
        ...
        this.canvas.width = this.map.clientWidth;
        this.canvas.height = this.map.clientHeight;
        this.projection.setSize(this.canvas.width, this.canvas.height);
        
        if ( this.updateTimer )
            clearTimeout(this.updateTimer);
        this.updateTimer = setTimeout(function(){
            var hw = self.canvas.width/2;
            var hh = self.canvas.height/2;
            var nw = self.projection.point([-hw,-hh]);
            var se = self.projection.point([hw,hh]);
            var h = Math.pow(2,Math.floor(self.zoom));
            var v = h/2;
            var tile_range = 360/h;

            var min_x = Math.floor((nw[0]+180)/tile_range);
            var max_x = Math.floor((se[0]+180)/tile_range);
            if ( max_x-min_x >= h ) {
                min_x = 0;
                max_x = h-1;
            }
            var min_y = 0, max_y = 0;
            if ( v >= 1 ) {
                min_y = v-Math.floor((90-se[1])/tile_range)-1;
                max_y = v-Math.floor((90-nw[1])/tile_range)-1;
            }
            if ( min_y < 0 )
                min_y = 0;
            if ( max_y < 0 )
            ...
            function addTile(context, z, y, x) {
                ...
                z = Math.floor(parseFloat(z));
                var zz = Math.pow(2,z);
                var xx = parseFloat(x)/zz;
                var yy = parseFloat(y)/zz;
                var offset = 1.0/(zz*256);
            ...
            //console.log("N="+(ne.lat()*60.0)+" S="+(sw.lat()*60.0)+" E="+(ne.lng()*60.0)+" W="+(sw.lng()*60.0));
            WRAP.Geo.updateBounds(new WRAP.Geo.Bounds(nw[1]*60.0, se[1]*60.0, se[0]*60.0, nw[0]*60.0));
            WRAP.Geo.render();
...
//    var view_timer;
    this.canvas.onmousemove = function(e) {
        self.adjustXY(e);
        if ( self._drag ) {
            var dx = self._mx - self._drag_x;
            var dy = self._my - self._drag_y;
            var cx = this.width/2;
            var cy = this.height/2;
            var mp = WRAP.Geo.getPoint(new WRAP.Geo.ScreenPoint(cx-dx,cy-dy));
            WRAP.Geo.setCenterPoint(mp);
            self._drag_x = self._mx;
            self._drag_y = self._my;
        }
        else {
        }
//        e.preventDefault();
    ...
    this.canvas.onmousedown = function(e) {
        self.adjustXY(e);
        self._drag = true;
        self._drag_offset = self._content_offset;
        self._drag_x = self._mx;
        self._drag_y = self._my;
//        e.preventDefault();
    }
    ...
    this.canvas.onmousewheel = function(e) {
        ...
        var d = e.wheelDelta/400;
        var z = WRAP.Geo.getZoom();
        z += d;
        if ( z < 0 )
            z = 0;
        if ( z > 10 )
...
*/
    ...
    this.getScreenPoint = function(pt) {
        if ( this.projection ) {
            ...
            var sp = this.projection.screenPoint([pt.lonDegree(), pt.latDegree()]);
            return new WRAP.Geo.ScreenPoint(parseInt(cx+sp[0]), parseInt(cy+sp[1]));
        }
        return new WRAP.Geo.ScreenPoint(0,0);
    }

    this.getScreenLine = function(/*path, n, s, w, e*/) {
console.log("##getScreenLine");
/*
        var lines = [];
        if ( this.projection ) {
            var z = Math.pow(2, this.zoom);
...
*/
    ...
    this.getPoint = function(pt) {
        if ( this.projection ) {
            var cx = this.canvas.width/2;
            var cy = this.canvas.height/2;
            var gp = this.projection.point([pt.x-cx, pt.y-cy]);
            return new WRAP.Geo.Point(gp[1]*60.0, gp[0]*60.0);
        }
        return new WRAP.Geo.Point(0,0);
    }

    this.getCenterPoint = function() {
        return new WRAP.Geo.Point(this.center_lat*60.0, this.center_lon*60.0);
    }

    this.setCenterPoint = function(pt) {
        this.center.lat = pt.latDegree();
        this.center.lon = pt.lonDegree();
        this.projection.setCenter(this.center);
        this.updateBounds();
    }
...
//console.log("##enableScroll");
    ...
    this.normalizeLon = function(lon) {
        while ( lon < -180 )
            lon += 360.0;
        while ( lon >= 180 )
            lon -= 360.0;
    ...
    this.setProjection('EQR');
}


WRAP.Geo.WebGL = {
    
    orthogonalMatrix: function(left, right, bottom, top, near, far) {
        ...
        m[15] =  1.0;
        m[ 1] = m[ 2] = m[ 3] = m[ 4] =
        m[ 6] = m[ 7] = m[ 8] = m[ 9] = m[11] = 0.0;
        return m;
    },
    
    setup: function(gl, vs, fs) {
        var vsh = gl.createShader(gl.VERTEX_SHADER);
        gl.shaderSource(vsh, vs);
        gl.compileShader(vsh);
        if (!gl.getShaderParameter(vsh, gl.COMPILE_STATUS))
            console.log("Vertex Shader Error : " + gl.getShaderInfoLog(vsh));
        var fsh = gl.createShader(gl.FRAGMENT_SHADER);
        gl.shaderSource(fsh, fs);
        gl.compileShader(fsh);
        if (!gl.getShaderParameter(fsh, gl.COMPILE_STATUS))
            console.log("Fragment Shader Error : " + gl.getShaderInfoLog(fsh));
    ...
    Projection: {
        ...
        EQR: function(gl) {
            ...
            this.setZoom = function(zoom) {
                this.zoom = zoom;
                this.ppd = (Math.pow(2,zoom)*256)/360;
                this.dpp = 1.0/this.ppd;
            }
            ...
            this.point = function(sp) {
                return [this.center.lon+sp[0]*this.dpp, this.center.lat-sp[1]*this.dpp];
            }
            
            this.screenPoint = function(gp) {
                return [(gp[0]-this.center.lon)*this.ppd, (this.center.lat-gp[1])*this.ppd];
            }
            
            this.Context = function(projection, gl) {
                this.revision = 0;
                this.tiles = [];
...
//                    console.log(" bounds n="+(bounds.north/60)+" s="+(bounds.south/60)+" w="+(bounds.west/60)+" e="+(bounds.east/60));
                    //            var canvas = document.createElement("canvas");
                    //            canvas.width = 256;
                    //            canvas.height = 256;
                    //            var ctx = canvas.getContext('2d');
                ...
                this._drawLines = function(lines, lineWidth, lineDash, strokeStyle, fillStyle, fillImage, option, id) {
                    ...
                    var DashPattern = function(array) {
                        this.array = array;
                        this.index = 0;
                        this.count = 0;
                    ...
                    function makeSegmentArray2(points, radius) {
                        var num = points.length;
                        var SegArr = [];
                        var firstPQ = makeSegment(points[0], points[1], radius);
                        SegArr.push(firstPQ[0]);
                        SegArr.push(firstPQ[1]);
                        
                        for ( var i = 0; i < num-2; i++) {
                            var PQ = makeSegment2_2(points[i], points[i+1], points[i+2], radius)
                            for ( var j = 0; j < PQ.length; j++ )
                                SegArr.push(PQ[j]);
                        }
                        
                        var lastPQ = makeSegment(points[num-1], points[num-2], radius);
                        SegArr.push(lastPQ[1]);
                        SegArr.push(lastPQ[0]);
                        
                        return SegArr;
                    }
                    
                    function cutPoints(points, dash) {
                        var head = [];
                        head.push(points[0]);
                        while(1) {
                            if ( points.length < 2 ) {
                            ...
                            var p1 = points[0];
                            var p2 = points[1];
                            var len = Math.sqrt((p1[0]-p2[0])*(p1[0]-p2[0])+(p1[1]-p2[1])*(p1[1]-p2[1]));
                            ...
                            if ( len < dash ) {
                                ...
                                var x = p1[0]+(p2[0]-p1[0])*dash/len;
                                var y = p1[1]+(p2[1]-p1[1])*dash/len;
                                points.shift();
                                points.unshift([x, y, p1[2]]);
                                head.push([x, y, p1[2]]);
                                return head;
                            }
                        }
                    ...
                    function makeDashedArray(points, radius, pattern) {
                        var shortPoints = reduceArray(points);
                        
                        if ( !pattern.safe() )
                            return makeSegmentArray2(shortPoints, radius);
                        
                        var DashArr = [];
                        while(1) {
                            var length = pattern.get();
                            var arr = cutPoints(shortPoints, length);
                            if ( pattern.active() )
                                DashArr.push(arr);
                            if ( shortPoints.length <= 1 )
                                break;
                        }
                        
                        var SegArr = [];
                        for ( var i = 0; i < DashArr.length; i++) {
                            var arr = makeSegmentArray2(DashArr[i], radius);
                            if ( i > 0 )
                                SegArr.push(arr[0]);
                            for ( var j = 0 ; j < arr.length ; j++ )
                                SegArr.push(arr[j]);
                            if ( i < DashArr.length-1 )
                                SegArr.push(arr[arr.length-1]);
                        }
                        return SegArr;
                    }
                    ...
                    function reduceArray(array) {
                        ...
                        for ( var i = 0; i < array.length-1; i++) {
                            if ( (array[i][0]-array[i+1][0])*(array[i][0]-array[i+1][0])+(array[i][1]-array[i+1][1])*(array[i][1]-array[i+1][1]) > min*min )
                                newArray.push(array[i+1]);
                        }
                        return newArray;
                    }
                    
                    function makeSegment(A, B, radius) {
                        var dx = B[0]-A[0];
                        var dy = B[1]-A[1];
                        var lenAB = Math.sqrt(dx*dx+dy*dy);
                        var vx = -dy*radius/lenAB;
                        var vy = dx*radius/lenAB;
                        var px = A[0]+vx;
                        var py = A[1]+vy;
                        var qx = A[0]-vx;
                        var qy = A[1]-vy;
                        return [[px, py, A[2]], [qx, qy, A[2]]] // 点P座標と点Q座標の配列
                    }
                    
                    function makeSegment2_2(A, B, C, radius) {
                        var ABx = B[0]-A[0];
                        var ABy = B[1]-A[1];
                        var BCx = C[0]-B[0];
                        var BCy = C[1]-B[1];
                        var lenAB = Math.sqrt(ABx*ABx+ABy*ABy);
                        var lenBC = Math.sqrt(BCx*BCx+BCy*BCy);
                        var vx = -ABy*radius/lenAB;
                        var vy = ABx*radius/lenAB;
                        var wx = -BCy*radius/lenBC;
                        var wy = BCx*radius/lenBC;
                        var cross = ABx*BCy-ABy*BCx;
                        
                        if ( cross < -min ) {
                            var s = (-lenBC*radius-vx*BCy+vy*BCx)/cross;
                            var qx = B[0]-vx-s*ABx;
                            var qy = B[1]-vy-s*ABy;
                            var lenPtoB = Math.sqrt(radius*radius+s*s*lenAB*lenAB);
                            var p1x = B[0]+(vx+s*ABx)*radius/lenPtoB;
                            var p1y = B[1]+(vy+s*ABy)*radius/lenPtoB;
                            var p2x = B[0]+vx;
                            var p2y = B[1]+vy;
                            var p3x = B[0]+wx;
                            var p3y = B[1]+wy;
                            if ( (qx-A[0])*ABx+(qy-A[1])*ABy < 0 || (qx-C[0])*BCx+(qy-C[1])*BCy > 0 )
                                return [[p2x, p2y, B[2]], [B[0]-vx, B[1]-vy, B[2]], [p1x, p1y, B[2]], B, [p3x, p3y, B[2]], [B[0]-wx, B[1]-wy, B[2]]];
                            return [[p2x, p2y, B[2]], [qx, qy, B[2]], [p1x, p1y, B[2]], [qx, qy, B[2]], [p3x, p3y, B[2]], [qx, qy, B[2]]];
                            
                        } else if ( cross > min ) {
                            var s = (-lenBC*radius-vx*BCy+vy*BCx)/cross;
                            var px = B[0]+vx+s*ABx;
                            var py = B[1]+vy+s*ABy;
                            var lenQtoB = Math.sqrt(radius*radius+s*s*lenAB*lenAB);
                            var q1x = B[0]-(vx+s*ABx)*radius/lenQtoB;
                            var q1y = B[1]-(vy+s*ABy)*radius/lenQtoB;
                            var q2x = B[0]-vx;
                            var q2y = B[1]-vy;
                            var q3x = B[0]-wx;
                            var q3y = B[1]-wy;
                            if ( (px-A[0])*ABx+(py-A[1])*ABy < 0 || (px-C[0])*BCx+(py-C[1])*BCy > 0 )
                                return [[B[0]+vx, B[1]+vy, B[2]], [q2x, q2y, B[2]], B, [q1x, q1y, B[2]], [B[0]+wx, B[1]+wy, B[2]], [q3x, q3y, B[2]]];
                            return [[px, py, B[2]], [q2x, q2y, B[2]], [px, py, B[2]], [q1x, q1y, B[2]], [px, py, B[2]], [q3x, q3y, B[2]]];
                            
                        } else if ( ABx*BCx+ABy*BCy < 0 ) {
                            var p1 = [B[0]+vx, B[1]+vy, B[2]];
                            var p2 = [B[0]+wx, B[1]+wy, B[2]];
                            var p3 = [B[0]+(vy-wy)/2, B[1]+(-vx+wx)/2, B[2]];
                            //var p4 = [B[0]-(vy-wy)/2, B[1]-(-vx+wx)/2, B[2]];
                            return [p1, p2, p3, B, p2, p1];
                        } else {
                            var p = [B[0]+(vx+wx)/2, B[1]+(vy+wy)/2, B[2]];
                            var q = [B[0]-(vx+wx)/2, B[1]-(vy+wy)/2, B[2]];
                            return [p, q];
                        }
                    }
                    
                    if ( strokeStyle ) {
                        var color = this.getColor(strokeStyle);
                        var shader = projection.colorShader;
                        if ( !dc.list )
                            dc.list = [];
                        var el = dc.list;
                        var e = el.length?el[el.length-1]:null;
                        if ( lineWidth <= 1 && (!lineDash || !lineDash.length) ) {
                            if ( !e || e.shader != shader || e.method != 'lineStrip' ) {
                                e = { shader:shader, method:'lineStrip', vertices:[], elements:[] };
                                el.push(e);
                            }
                            var v = e.vertices;
                            ...
                            for ( var i = 0 ; i < lines.length ; i++ ) {
                                ...
                                for ( var j = 0 ; j < line.length ; j++ ) {
                                    var p = line[j];
                                    var pt = projection.screenPoint(p);
                                    array.push([pt[0], pt[1], j]);
                                }
                                var pattern = new DashPattern(lineDash);

                                var res = makeDashedArray(array, lineWidth, pattern);
                                ...
                                for ( var j = 0 ; j < res.length ; j++ ) {
                                    ...
                                    var a = line[index];
                                    var s = array[index];
                                    vtx.push([a[0], a[1], v[0]-s[0], -v[1]+s[1]]);
                                }
                                
                                if ( !e || e.shader != shader || e.method != 'triangleStrip' ) {
                                    e = { shader:shader, method:'triangleStrip', vertices:[], elements:[] };
                                    el.push(e);
                                }
                                var v = e.vertices;
                                var line = lines[i];
...
*/
                    ...
                    if ( fillStyle ) {
                        var color = this.getColor(fillStyle);
                        var shader = projection.colorShader;
                        if ( !dc.list )
                            dc.list = [];
                        var el = dc.list;
                        var e = el.length?el[el.length-1]:null;
                        if ( !e || e.shader != shader || e.method != 'triangles' ) {
                            e = { shader:shader, method:'triangles', vertices:[], elements:[] };
                            el.push(e);
                        }
                        ...
                        for ( var i = 0 ; i < lines.length ; i++ ) {
                            ...
                            var pa = new Float32Array(line.length*2);
                            for ( var j = 0 ; j < line.length ; j++ ) {
                                pa[j*2] = line[j][0];
                                pa[j*2+1] = line[j][1];
                            }
                        ...
                        var vi = gp.tessellate(contours);

                        if ( vi && vi.triangles.length ) {
                            e.elements.push({
                                index:v.length/9,
                                num:vi.triangles.length
                            });
                            for ( var j = 0 ; j < vi.triangles.length ; j++ ) {
                                var index = vi.triangles[j]*2;
                                v.push(vi.vertices[index]);
                                v.push(vi.vertices[index+1]);
                                v.push(color[0]);
                                v.push(color[1]);
                                v.push(color[2]);
                                v.push(color[3]);
...
WRAP.Geo.check(lineWidth, lineDash, option);
/*
                    var ctx = this.ctx;
                    var op = map.getScreenPoint(new WRAP.Geo.Point(0,0));
                    var cx = map.getSize().width/2;
                    ...
                    for ( var i = 0 ; i < path.length ; i++ ) {
                        ...
                        if ( i > 0 ) {
                            ...
                            if ( d == 0 ) {
                                ...
                                var r = (lat2-lat1)/(lon2-lon1);
                                if ( lon1 < 180 && 180 < lon2 ) {
                                    var d = 180-lon1;
                                    var lat = lat1 + d*r;
                                    fragments.push([lon1, lat1]);
                                    fragments.push([180, lat]);
                                    fragments.push([180, lat]);
                                }
                                else {
                                    fragments.push([lon1, lat1]);
...
//                    console.log("min="+min+" max="+max);
                    if ( min < -180.0 ) {
                        var a = [];
                        for ( var i = 0 ; i < fragments.length ; i++ ) {
                            var p = [fragments[i][0]+360, fragments[i][1]];
                            a.push(p);
                        }
                        lines.push(a);
                    }
                        ...
                        if ( Array.isArray(path[0][0]) ) {
                            for ( var i = 0 ; i < path.length ; i++ ) {
                                ...
                                if ( geodesic )
                                    ...
                                    fragments = this._normalizePath(path[i]);
                                this._addFragments(lines, fragments);
                            }
                            if ( !lines.length )
                                return;
                            ...
                            if ( geodesic )
                                ...
                                fragments = this._normalizePath(path);
                            this._addFragments(lines, fragments);
                        }
                        this._drawLines(lines, lineWidth, lineDash, strokeStyle, fillStyle, c.image, option, id);
                    }
                ...
                this.drawPoint = function(point, size, lineWidth, strokeStyle, fillStyle) {
                    ...
                    var shader = projection.pointShader;
                    if ( !dc.list )
                        dc.list = [];
                    var el = dc.list;
                    var e = el.length?el[el.length-1]:null;
                    if ( !e || e.shader != shader ) {
                        e = { shader:shader, method:'points', vertices:[], elements:[{index:0, num:0}] };
                        el.push(e);
                    }
...
*/
                ...
                this.setLayer = function() {
                    if ( !this.layers.length || this.layers[this.layers.length-1] != this.layer )
                ...
                this.end = function(/*revision*/) {
                    ...
                    var ppd = projection.ppd;
                    var band = 360*ppd;
                    var w = projection.width;
                    var h = projection.height;
                    var hw = w/2;
                    var hh = h/2;
                    
                    var t = hh + (90.0 - projection.center.lat)*ppd;
                    var b = hh - (projection.center.lat + 90.0)*ppd;
                    var l = hw - (projection.center.lon + 180)*ppd;
                    var r = hw + (180 - projection.center.lon)*ppd;
                    
                    var shifts = [0];
                    var shift = 0;
                    while ( 0 < l+shift ) { // left side repeat
                        shift -= band;
                        shifts.push(shift);
                    }
                    shift = 0;
                    while ( r+shift < w ) { // right side repeat
                        shift += band;
                        shifts.push(shift);
                    }
                    ...
                    for ( var c = 0 ; c < shifts.length ; c++ ) {
                        ...
                        var vw = r-l;
                        var matrix = gp.orthogonalMatrix(-vw/2,vw/2,-vh/2,vh/2,-100.0,100.00);
                        // adjust rendering center point
                        var offset_x = (w-vw)/2;
                        var offset_y = (h-vh)/2;
                        matrix[3] = (-l+shift+offset_x)/(vw/2);
                        matrix[7] = (-b+offset_y)/(vh/2);
                        
                        gl.viewport(l, b, vw, vh);
                        
                        //console.log("l="+l+" r="+r+" t="+t+" b="+b+" ox="+(l+hw)+" oy="+(b+hh)+" w="+vw+" h="+vh);
                        
                        for ( var i = 0 ; i < this.layers.length ; i++ ) {
                            var layer = this.layers[i];
                            if ( layer._dc ) {
                                var el = layer._dc.list;
                                for ( var j = 0 ; j < el.length ; j++ ) {
                                    var e = el[j];
                                    if ( !e.vbo ) {
                                        e.vbo = e.shader.createBuffer();
                                        e.shader.set(e.vbo, e.vertices);
                                        e.vertices = null;
                                    }
                                    e.shader[e.method](matrix, e.vbo, e.elements);
                                }
                                layer._dc.cached = true;
                            }
                        }
                    }
                    gl.flush();
                ...
                this.getColor = function(style) {
                    if ( !this.color_cache )
                        this.color_cache = {};
                    var color = this.color_cache[style];
                    if ( !color ) {
                        var c = WRAP.Geo.parseColor(style);
                        color = [];
                        color[0] = parseFloat(c[0])/255.0;
                        color[1] = parseFloat(c[1])/255.0;
                        color[2] = parseFloat(c[2])/255.0;
                        color[3] = parseFloat(c[3])/255.0;
                        this.color_cache[style] = color;
                    }
                    return color;
                }
            ...
            this.ColorShader = function() {
                
                var ctl = WRAP.Geo.WebGL;
                this.pg = ctl.setup(gl,
                    ...
                    "void main()"+
                    "{"+
                    "    float y = (position.y-center_lat)*scale;"+
                    "    float x = (position.x-center_lon)*scale;"+
                    "    vec4 p = vec4(x+offset.x, y+offset.y, 0.0, 1.0);"+
                ...
                // accessor
                //var coordinate = gl.getAttribLocation(this.pg, "coordinate");
                var offset = gl.getAttribLocation(this.pg, "offset");
                var position = gl.getAttribLocation(this.pg, "position");
                var color = gl.getAttribLocation(this.pg, "color");
                var projection = gl.getUniformLocation(this.pg, "projection");
                var center_lat = gl.getUniformLocation(this.pg, "center_lat");
                var center_lon = gl.getUniformLocation(this.pg, "center_lon");
                var scale = gl.getUniformLocation(this.pg, "scale");
...
//                    gl.useProgram(this.pg);
                ...
                this.deleteBuffer = function(vbo) {
//                    gl.useProgram(this.pg);
                    gl.deleteBuffer(vbo);
                }
                
                this.set = function(vbo, vts) {
//                    gl.useProgram(this.pg);
                    gl.bindBuffer(gl.ARRAY_BUFFER, vbo);
                    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(vts), gl.DYNAMIC_DRAW);
//                    gl.bufferSubData(gl.ARRAY_BUFFER, 0, new Float32Array(vts));
                }
                
                this.draw = function(type, proj, vbo, elements) {
                    gl.useProgram(this.pg);
                    
                    gl.uniformMatrix4fv(projection, false, proj);
                    gl.uniform1f(center_lat, projction.center.lat);
                    gl.uniform1f(center_lon, projction.center.lon);
                    gl.uniform1f(scale, projction.ppd);
                    
                    var floatsize = 4;
                    gl.bindBuffer(gl.ARRAY_BUFFER, vbo);
                    gl.vertexAttribPointer(position, 2, gl.FLOAT, false, 9*floatsize, 0);
                    gl.enableVertexAttribArray(position);
                    gl.vertexAttribPointer(color, 4, gl.FLOAT, false, 9*floatsize, 2*floatsize);
                    gl.enableVertexAttribArray(color);
                    gl.vertexAttribPointer(offset, 2, gl.FLOAT, false, 9*floatsize, 7*floatsize);
                    gl.enableVertexAttribArray(offset);
                    
                    for ( var i = 0 ; i < elements.length ; i++ ) {
                        var e = elements[i];
                        gl.drawArrays(type, e.index, e.num);
                    }
                }
                
                this.triangleStrip = function(proj, vbo, elements) {
                    this.draw(gl.TRIANGLE_STRIP, proj, vbo, elements);
                }
                
                this.triangles = function(proj, vbo, elements) {
                    this.draw(gl.TRIANGLES, proj, vbo, elements);
                }
                
                this.lineStrip = function(proj, vbo, elements) {
                    this.draw(gl.LINE_STRIP, proj, vbo, elements);
                }
                
            }
            
            this.PointShader = function() {
                ...
                var position = gl.getAttribLocation(this.pg, "position");
                var color = gl.getAttribLocation(this.pg, "color");
                var value = gl.getAttribLocation(this.pg, "value");
                var projection = gl.getUniformLocation(this.pg, "projection");
                var center_lat = gl.getUniformLocation(this.pg, "center_lat");
                var center_lon = gl.getUniformLocation(this.pg, "center_lon");
                var scale = gl.getUniformLocation(this.pg, "scale");
...
//                    gl.bufferSubData(gl.ARRAY_BUFFER, 0, new Float32Array(vts));
                ...
                this.draw = function(type, proj, vbo, elements) {
                    ...
                    gl.vertexAttribPointer(color, 4, gl.FLOAT, false, 9*floatsize, 2*floatsize);
                    gl.enableVertexAttribArray(color);
                    gl.vertexAttribPointer(value, 1, gl.FLOAT, false, 9*floatsize, 6*floatsize);
                ...
                this.points = function(proj, vbo, elements) {
                    this.draw(gl.POINTS, proj, vbo, elements);
                }
            }
            
            this.colorShader = new this.ColorShader();
            this.pointShader = new this.PointShader();
            
        }
        
    },
    
    tessellate: function(loops) {
...
WRAP.Geo.Renderer.Mesh = function() {
    ...
    this.setPalette = function(style) {
        if ( !self.palette && style.palette ) {
            ...
            var m = 0;
            for ( var v = self.palette_value_min ; v <= (self.palette_value_max+self.palette_step) ; v+=self.palette_step ) {
    ...
    this.getColor = function(value) {
        ...
        if ( index < 0 )
            return [0,0,0,0];
        if ( index >= self.palette.length )
            index = self.palette.length-1;
    ...
    this.draw = function(layer, content, conf, context, style, data, revision, dl) {
        ...
        var data_scale = conf.Attributes.data_scale || 1;
...
//console.log("rendering Mesh");
        ...
        var blend_tile = [];
        var blend_layer = layer.getBlendLayer();
        if ( blend_layer ) {
            for ( var j = 0 ; j < blend_layer.length ; j++ ) {
                var bl = blend_layer[j];
                if ( !bl.visible() )
                    continue;
                var blend_data_tile = [];
                for ( var i = 0 ; i < tiles.length ; i++ ) {
                    var tile = tiles[i];
                    if ( !(blend_data_tile[i] = bl._data._handler().getTile(content, tile.id, sampling)) ) {
                        bl._data._handler().loadTile(content, tile.id, sampling, tile.z, tile.y, tile.x, tile.cood, tile.bounds,
                            function() {
                                WRAP.Geo.redraw(layer);
                        });
                        return;
                ...
                blend_tile.push(blend_data_tile);
            }
        }
...
//console.log("tiles="+tiles.length+" drawn="+drawn.length);
        ...
        for ( var i = 0 ; i < data_tile.length ; i++ ) {
            ...
            var s = null;
            if ( src && !src.empty )
                s = this.getData(content, src.data);
            
            if ( blend_tile.length && s ) {
                s = s.concat();
            }
            
            for ( var j = 0 ; j < blend_tile.length ; j++ ) {
                var blend_src = blend_tile[j][i];
                if ( !blend_src || blend_src.empty )
                    continue;
                if ( !s )
                    s = new Array(256*256);
                var b = this.getData(content, blend_src.data);
                if ( !b )
                    continue;
            ...
            if ( fill ) {
                for ( var m = 0 ; m < 65536 ; m++ ) {
                    ...
                    if ( isNaN(value) )
                        continue;
                    value = value * data_scale + data_offset;
...
//console.log("tile="+tile.id+" min="+min+" max="+max);
            ...
            if ( contour ) {
                ...
                for ( var j = 0 ; j < contour.length ; j++ ) {
                    ...
                    var unit_label = c.unit_label || "";
                    var digits = (c.fractional_digits===undefined)?undefined:parseInt(c.fractional_digits);

                    var interval = c.interval/data_scale;
                    var display_value = c.base;
                    var value = (c.base - data_offset)/data_scale;
                    for ( var n = 0 ; n < c.num ; n++ ) {
                        if ( min <= value && value <= max ) {
                            ...
                            if ( 20 < lx && lx <= 236 && 20 < ly && ly <= 236 ) {
                                var label_value = (digits === undefined)?display_value:display_value.toFixed(digits);
                                label.push({x:lx, y:ly, value:label_value+unit_label, color:c.textColor});
                            }
                        }
                        display_value += c.interval;
                        value += interval;
                    }
                }
            }
            ...
            for ( var l = 0 ; l < label.length ; l++ ) {
                var center = (label[l].y*256+label[l].x)*2;
                var lat = tile.cood[center];
                var lon = tile.cood[center+1];
        ...
        for ( var m = 0 ; m < tl.length ; m++ ) {
            ...
            if ( f.data && f.data.label ) {
                ...
                for ( var l = 0 ; l < label.length ; l++ ) {
                    layer.addFeature(label[l]);
                }
            }
        }
        
        //WRAP.Geo._tileCheck(context, layer);
    ...
    this.draw = function(layer, content, conf, context, style, data, revision, dl) {
        ...
        for ( var i = 0 ; i < data_tile.length ; i++ ) {
            ...
            for ( var j = 0 ; j < data.length ; j++ ) {
                ...
                var text = (v.value*data_scale+data_offset).toFixed(digit);
                var feature;
                feature = new WRAP.Geo.Feature.Text({
                    point:[v.lon,v.lat],
                    text:text,
...
WRAP.Geo.Renderer.WindArrow = function() {
    ...
    this.draw = function(layer, content, conf, context, style, data, revision/*, dl*/) {
        ...
        var data_scale = conf.Attributes.data_scale || 1;
        var data_offset = conf.Attributes.data_offset || 0;
        var threshold = conf.Attributes.threshold || 0;
        ...
        for ( var i = 0 ; i < data_tile.length ; i++ ) {
            ...
            for ( var j = 0 ; j < data.length ; j++ ) {
                ...
                var u_spd = v.value[0]*data_scale+data_offset;
                var v_spd = v.value[1]*data_scale+data_offset;
...
WRAP.Geo.Renderer.GridArrow = function() {
    var self = this;
    ...
    this.setPalette = function(style) {
        if ( !self.color && style.color ) {
            self.color = WRAP.Geo.parseColor(style.color);
        }
        if ( !self.palette && style.palette ) {
            self.palette = [];
            self.palette_gradient = style.palette_gradient;
    ...
    this.draw = function(layer, content, conf, context, style, data, revision/*, dl*/) {
        ...
        var data_scale = conf.Attributes.data_scale || 1;
        var data_offset = conf.Attributes.data_offset || 0;
        var data_type = conf.Attributes.type;
        var threshold = conf.Attributes.threshold || 0;
        
        var base_length = s.base_length || 16;
        var speed_length_racio = s.speed_length_racio || 0;
        ...
        for ( var i = 0 ; i < data_tile.length ; i++ ) {
            ...
            if ( blend_tile.length && data ) {
                data = data.concat();
            }
            
            for ( var j = 0 ; j < blend_tile.length ; j++ ) {
                var blend_data = blend_tile[j][i].data;
                if ( !blend_data )
                    continue;
                if ( !data )
                    data = new Array();
                for ( var k = 0 ; k < blend_data.length ; k++ ) {
                    var bv = blend_data[k];
                    if ( bv.value === undefined || isNaN(bv.value) )
                        continue;
                    for ( var l = 0 ; l < data.length ; l++ ) {
                        var dv = data[l];
                        if ( dv.lat == bv.lat && dv.lon == bv.lon ) {
                            data.splice(l,1);
                            break;
                        }
                    }
            ...
            for ( var j = 0 ; j < data.length ; j++ ) {
                var v = data[j];
                var speed = 0, dir = 0;
                if ( data_type == 'SpeedDirection' ) {
                    if ( !v.value || v.value.length != 2 || isNaN(v.value[0]) || isNaN(v.value[1]) )
                        continue;
                    speed = v.value[0] * data_scale + data_offset;
                    dir = v.value[1];
                }
                else if ( data_type == 'Direction' ) {
                    if ( v.value === undefined || isNaN(v.value) )
                        continue;
                    speed = -1;
                    dir = v.value * data_scale + data_offset;
                }
                else {
                    if ( !v.value || v.value.length != 2 || isNaN(v.value[0]) || isNaN(v.value[1]) )
                        continue;
                    var u_spd = v.value[0];
                    var v_spd = v.value[1];
                    speed = Math.sqrt(u_spd*u_spd + v_spd*v_spd) * data_scale + data_offset;
                ...
                if ( speed < 0 || speed >= threshold ) {
                    var col = this.getColor(speed);
                    
                    var l = base_length+speed*speed_length_racio;
                    var feature = new WRAP.Geo.Feature.Line({
                        point:[v.lon,v.lat],
                        line:[[[0,0],[0,l]],[[2,l-4],[0,l],[-2,l-4]]],
                        strokeStyle:"rgba("+col[0]+","+col[1]+","+col[2]+","+(col[3]/255)+")",
                        rotation:dir
                    });
                    layer.addFeature(feature);
/*
...
*/
/*
//                  var text = speed.toFixed(2);
                    var text = dir.toFixed(2);
                    feature = new WRAP.Geo.Feature.Text({
                        point:[v.lon,v.lat],
                        text:text,
                        fontSize:14,
...
WRAP.Geo.Renderer.GridLine = function() {
    ...
    this.draw = function(layer, content, conf, context, style, data, revision/*, dl*/) {
        ...
        for ( h = 0 ; h < lon.length ; h++ ) {
            var x = lon[h].lon;
            layer.addFeature(new WRAP.Geo.Feature.GeoLine({
                path:[[x,vl],[x,-vl]],
        ...
        for ( h = 0 ; h < lon.length ; h++ ) {
            ...
            if ( (ll=lon[h].top) && ll.d < 4 ) {
                layer.addFeature(new WRAP.Geo.Feature.Text({
...
WRAP.Geo.Renderer.TropicalStorm = function() {
    ...
    this.draw = function(layer, content, conf, context, style, data, revision, dl) {
        ...
        var s = conf.Attributes;
        
        function drawLine(p, r1, r2, dir, line_color, line_width) {
            if ( !r1 && !r2 )
                return;
            var s = WRAP.Geo.getGCPoint(p, r1, dir);
            var e = WRAP.Geo.getGCPoint(p, r2, dir);
            var feature = new WRAP.Geo.Feature.GeoLine({
                path:[[s.lonDegree(),s.latDegree()],[e.lonDegree(),e.latDegree()]],
                strokeStyle:line_color,
                lineWidth:line_width
            });
            layer.addFeature(feature);
        ...
        function drawWindSector(point, prop) {
            var factor = 1852;
            var r35_1 = (prop.stmrds_35_1 && prop.stmrds_35_1*factor) || 0;
            var r35_2 = (prop.stmrds_35_2 && prop.stmrds_35_2*factor) || 0;
            var r35_3 = (prop.stmrds_35_3 && prop.stmrds_35_3*factor) || 0;
            var r35_4 = (prop.stmrds_35_4 && prop.stmrds_35_4*factor) || 0;
            var r50_1 = (prop.stmrds_50_1 && prop.stmrds_50_1*factor) || 0;
            var r50_2 = (prop.stmrds_50_2 && prop.stmrds_50_2*factor) || 0;
            var r50_3 = (prop.stmrds_50_3 && prop.stmrds_50_3*factor) || 0;
            var r50_4 = (prop.stmrds_50_4 && prop.stmrds_50_4*factor) || 0;
            
            if ( r35_1 ) {
                var feature = new WRAP.Geo.Feature.GeoArc({
                    point:point,
                    radius:r35_1,
                    start:prop.stmrda_35_1,
                    end:prop.stmrda_35_2,
                    strokeStyle:s.wind_line_color_35kt,
                    lineWidth:s.wind_line_width,
                    fillStyle:s.wind_fill_color_35kt,
                });
                layer.addFeature(feature);
            }
            if ( r35_2 ) {
                var feature = new WRAP.Geo.Feature.GeoArc({
                    point:point,
                    radius:r35_2,
                    start:prop.stmrda_35_2,
                    end:prop.stmrda_35_3,
                    strokeStyle:s.wind_line_color_35kt,
                    lineWidth:s.wind_line_width,
                    fillStyle:s.wind_fill_color_35kt,
                });
            ...
            if ( r35_3 ) {
                var feature = new WRAP.Geo.Feature.GeoArc({
                    point:point,
                    radius:r35_3,
                    start:prop.stmrda_35_3,
                    end:prop.stmrda_35_4,
                    strokeStyle:s.wind_line_color_35kt,
                    lineWidth:s.wind_line_width,
                    fillStyle:s.wind_fill_color_35kt,
                });
            ...
            if ( r35_4 ) {
                var feature = new WRAP.Geo.Feature.GeoArc({
                    point:point,
                    radius:r35_4,
                    start:prop.stmrda_35_4,
                    end:prop.stmrda_35_1+360,
                    strokeStyle:s.wind_line_color_35kt,
                    lineWidth:s.wind_line_width,
                    fillStyle:s.wind_fill_color_35kt,
                });
            ...
            var p = new WRAP.Geo.Point(point[1]*60.0, point[0]*60.0);
            drawLine(p, r35_1, r35_4, prop.stmrda_35_1, s.wind_line_color_35kt, s.wind_line_width);
            drawLine(p, r35_1, r35_2, prop.stmrda_35_2, s.wind_line_color_35kt, s.wind_line_width);
            drawLine(p, r35_2, r35_3, prop.stmrda_35_3, s.wind_line_color_35kt, s.wind_line_width);
            drawLine(p, r35_3, r35_4, prop.stmrda_35_4, s.wind_line_color_35kt, s.wind_line_width);
            
            if ( r50_1 ) {
                var feature = new WRAP.Geo.Feature.GeoArc({
                    point:point,
                    radius:r50_1,
                    start:prop.stmrda_50_1,
                    end:prop.stmrda_50_2,
                    strokeStyle:s.wind_line_color_50kt,
                    lineWidth:s.wind_line_width,
                    fillStyle:s.wind_fill_color_50kt,
                });
                layer.addFeature(feature);
            }
            if ( r50_2 ) {
                var feature = new WRAP.Geo.Feature.GeoArc({
                    point:point,
                    radius:r50_2,
                    start:prop.stmrda_50_2,
                    end:prop.stmrda_50_3,
                    strokeStyle:s.wind_line_color_50kt,
                    lineWidth:s.wind_line_width,
                    fillStyle:s.wind_fill_color_50kt,
                });
            ...
            if ( r50_3 ) {
                var feature = new WRAP.Geo.Feature.GeoArc({
                    point:point,
                    radius:r50_3,
                    start:prop.stmrda_50_3,
                    end:prop.stmrda_50_4,
                    strokeStyle:s.wind_line_color_50kt,
                    lineWidth:s.wind_line_width,
                    fillStyle:s.wind_fill_color_50kt,
                });
            ...
            if ( r50_4 ) {
                var feature = new WRAP.Geo.Feature.GeoArc({
                    point:point,
                    radius:r50_4,
                    start:prop.stmrda_50_4,
                    end:prop.stmrda_50_1+360,
                    strokeStyle:s.wind_line_color_50kt,
                    lineWidth:s.wind_line_width,
                    fillStyle:s.wind_fill_color_50kt,
                });
            ...
            drawLine(p, r50_1, r50_4, prop.stmrda_50_1, s.wind_line_color_50kt, s.wind_line_width);
            drawLine(p, r50_1, r50_2, prop.stmrda_50_2, s.wind_line_color_50kt, s.wind_line_width);
            drawLine(p, r50_2, r50_3, prop.stmrda_50_3, s.wind_line_color_50kt, s.wind_line_width);
            drawLine(p, r50_3, r50_4, prop.stmrda_50_4, s.wind_line_color_50kt, s.wind_line_width);
        }
        ...
        for ( var id in root ) {
            ...
            for ( var i = 0 ; i < geo.features.length ; i++ ) {
                ...
                var type = prop.wrap_feature_type;
            ...
            for ( var i = 0 ; i < geo.features.length ; i++ ) {
                ...
                if ( type == 'analysis' ) {
                    track.push(point);
                    forecast.unshift(point);
                }
            }
            
            var active_type = ['analysis'];
            
            if ( s.show_track_history === undefined || s.show_track_history === true ) {
                active_type.push('track');
                var feature = new WRAP.Geo.Feature.GeoLine({
                    path:track,
                    strokeStyle:s.track_history_line_color,
                    lineWidth:s.track_history_line_width
                });
                layer.addFeature(feature);
            }
            if ( s.show_forcast_track === undefined || s.show_forcast_track === true ) {
                active_type.push('forecast');
                var feature = new WRAP.Geo.Feature.GeoLine({
                    path:forecast,
                    strokeStyle:s.forecst_track_line_color,
                    lineWidth:s.forecst_track_line_width
                });
                layer.addFeature(feature);
            }
            if ( s.show_analysis_wind_radii === undefined || s.show_analysis_wind_radii === true ) {
                for ( var i = 0 ; i < geo.features.length ; i++ ) {
                    var f = geo.features[i];
                    var prop = f.properties;
                    if ( prop.wrap_feature_type == 'analysis' ) {
                        var point = f.geometry.coordinates;
                        drawWindSector(point, prop);
                    }
                }
            }
            if ( s.show_forcast_track === undefined || s.show_forcast_track === true ) {
                if ( s.show_forcast_wind_radii === undefined || s.show_forcast_wind_radii === true ) {
                    for ( var i = 0 ; i < geo.features.length ; i++ ) {
                        var f = geo.features[i];
                        var prop = f.properties;
                        if ( prop.wrap_feature_type == 'forecast' ) {
                            var point = f.geometry.coordinates;
                            drawWindSector(point, prop);
                        }
                    }
            ...
            for ( var i = 0 ; i < geo.features.length ; i++ ) {
                var f = geo.features[i];
                var prop = f.properties;
                if ( prop.wrap_feature_type == 'track' )
                    continue;
                if ( active_type.indexOf(prop.wrap_feature_type) < 0 )
                    continue;
                var date = prop.valid;
                if ( date && date.length >= 11 ) {
                    var text = date.substr(6,2);
                    text += " / ";
                    text += date.substr(9,2);
                    text += "Z";
                    var point = f.geometry.coordinates;
                    var feature = new WRAP.Geo.Feature.Text({
                        text:text,
                        point:point,
                        strokeStyle:s.label_edge_color,
                        fillStyle:s.label_text_color,
                        offsetX:s.label_offset_x,
                        offsetY:s.label_offset_y,
                        align:'left'
                    });
                    feature.geo = f;
                    layer.addFeature(feature);
            ...
            for ( var i = 0 ; i < geo.features.length ; i++ ) {
                ...
                var type = prop.class;
                var point = f.geometry.coordinates;
                var icon = s.icon[type];
                if ( point[1] < 0 && s.icon_south && s.icon_south[type] )
                    icon = s.icon_south[type];
                if ( icon ) {
                    var feature = new WRAP.Geo.Feature.Image({
                        image:icon,
                        point:point,
                        ...
                        offsetX:s.icon_offset_x,
                        offsetY:s.icon_offset_y,
                    });
                    feature.geo = f;
                    layer.addFeature(feature);
                }
