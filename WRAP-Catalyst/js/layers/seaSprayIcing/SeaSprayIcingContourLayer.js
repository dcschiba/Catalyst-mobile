...
import {
  ...
  SEA_SPRAY_ICING_TIMELIST } from '../../constants/seasprayicing/ActionTypes';

class SeaSprayIcingContourLayer extends WrapLayer {
  constructor(conf) {
    super(conf);
    this.timeRange = {};
  ...
  dataInspect(timelist) {
    if (timelist && timelist.length) {
      ...
      this.dispathfunc(SEA_SPRAY_ICING_TIMELIST, { basetime,
        tsobj,
        basetimeidx: 0,
        validtimeidx: 0 },
        this.name());
  ...
  ctrlLayer(type, state) {
    if (type === SEA_SPRAY_ICING_FLAT_CLICK) {
      return;
    }
    const { seaSprayIcingChecked,
            seaSprayIcingContourChecked,
            basetimeidx,
            validtimeidx } = state.seasprayicing;
    const enabled = seaSprayIcingChecked && seaSprayIcingContourChecked;
    if (enabled) {
      this.setContent(
        {
          element: 'TSEC',
    ...
    if (type === SEA_SPRAY_ICING_CLICK || type === SEA_SPRAY_ICING_CONTOUR_CLICK) {
      this.setVisible(enabled);
    }
  }
}

export default SeaSprayIcingContourLayer;
