...
class SeaSprayIcingFlatLayer extends WrapLayer {
  constructor(conf) {
    super(conf);
    this.timeRange = {};
  ...
  ctrlLayer(type, state) {
    if (type === SEA_SPRAY_ICING_CONTOUR_CLICK) {
      return;
    }
    ...
    const enabled = seaSprayIcingChecked && seaSprayIcingFlatChecked;
    if (enabled) {
      this.setContent(
        {
          element: 'TSEC',
    ...
    if (type === SEA_SPRAY_ICING_CLICK || type === SEA_SPRAY_ICING_FLAT_CLICK) {
      this.setVisible(enabled);
    }
  }
}

export default SeaSprayIcingFlatLayer;
