...
import { CURRENT_TIMELIST } from '../constants/current/ActionTypes';

class CurrentLayer extends WrapLayer {
  constructor(conf) {
    super(conf);
    this.timeRange = {};
  ...
  dataInspect(timelist) {
    if (timelist && timelist.length) {
      ...
      this.dispathfunc(CURRENT_TIMELIST, { basetime,
        tsobj,
        basetimeidx: 0,
        validtimeidx: 0 },
        this.name());
  ...
  ctrlLayer(type, state) {
    ...
    if (current.currentChecked) {
      this.setContent(
        { element: ['UOGRD', 'VOGRD'] },
        current.basetimeidx,
        current.validtimeidx,
      );
      this.setAttributes({ threshold: current.threshold });
    }
    this.setVisible(current.currentChecked);
  }
}
